                                    ...
                                    <div className="">
                                        <div className="tabs-menu ">
                                            <ul className="nav panel-tabs eductaional-tabs mb-6">
                                                <li className=""><a href="#tab1" className="active show" data-toggle="tab">Todos</a></li>
                                                {
                                                    (coursesByCategories && coursesByCategories.length > 0)? 
                                                        coursesByCategories.map((category, index) => (
                                                            (category.courses.length > 0)?
                                                                <li key={category.id}>
                                                                    <a key={category.id} href={`#tab${category.id + 1}`} data-toggle="tab" className="">{category.name}</a>
                                                                </li>
                                                            :
                                                            ""
                                                        ))
                                    ...
                                    <div className="panel-body">
                                        <div className="tab-content">
                                                                ...
                                                                <div className="card overflow-hidden">
                                                                    ...
                                                                    <div className="item-card7-img">
                                                                        <div className="item-card7-imgs custom-img">
                                                                            <Link to={"/courseDetails/"+course.id} onClick={() => onSeeCourseDetails(course)}/>
                                                                            <img src={process.env.REACT_APP_NODE_URL + '/uploads/courses/images/' + course.image} alt="img" className="img-height cover-image"/>
                                                                        </div>
                                                                        <div className="item-card7-overlaytext">
                                                                            <Link to={"/courseDetails/"+course.id} className="text-white">{course.category.name}</Link>
                                                                        </div>
                                                                    </div>
                                                                    <div className="card-body">
                                                                        <div className="item-card7-desc area-text">
                                                                            <div className="item-card7-text">
                                                                                <Link to={"/courseDetails/"+course.id} onClick={() => onSeeCourseDetails(course)} className="text-dark"><h3 className="font-weight-semibold">{course.name}</h3></Link>
                                                                            </div>
                                                                            <ul className="d-flex mb-2">
                                                                                <li><span className="icons text-muted"><i className="fa fa-calendar-check-o mr-1"></i>{getFullDateNoHours(course.begin_date)}</span></li>
                                                                                <li><span className="icons text-muted"><i className="fa fa-calendar-times-o mr-1"></i>{getFullDateNoHours(course.end_date)}</span></li>
                                                                            </ul>
                                                                    ...
                                                                    <div className="card-body p-4 pl-5">
                                                                        <a className="mr-4" href="/"> <span className="font-weight-bold">Duration :</span> <span className="text-muted">{course.duration} Hours</span></a>
                                                                        <a className="mr-4 float-right" href="/"><span className="font-weight-bold">Modality:</span><span className="text-muted"> {course.modality.name} </span></a>
                                                                    </div>
                                                                    <div className="card-body">
                                            ...
                                            </div>
                                            {
                                                (coursesByCategories && coursesByCategories.length > 0)? 
                                                  coursesByCategories.map((category, index) => (
                                                    <div key={category.id} className="tab-pane" id={`tab${category.id + 1 }`}>
                                                        <div className="row">
                                                        {
                                                                    ...
                                                                    <div key={course.id} className="col-xl-4 col-md-6">
                                                                        <div className="card overflow-hidden">
                                                                            ...
                                                                            <div className="item-card7-img">
                                                                                ...
                                                                                <div className="item-card7-overlaytext">
                                                                                    <Link to="/courseDetails" onClick={() => onSeeCourseDetails(course)} className="text-white">{course.category.name}</Link>
                                                                                </div>
                                                                            </div>
                                                                            <div className="card-body">
                                                                                <div className="item-card7-desc">
                                                                                    ...
                                                                                    <ul className="d-flex mb-2">
                                                                                        <li className=""><a href="/" className="icons text-muted"><i className="icon icon-location-pin  mr-1"></i> USA</a></li>
                                                                                        <li><a href="/" className="icons text-muted"><i className="icon icon-event  mr-1"></i>1 min ago</a></li>
                                                                                        <li className=""><a href="/" className="icons text-muted"><i className="icon icon-phone  mr-1"></i> 14 675 65</a></li>
                                                                                    </ul>
                                                                                    <p className="mb-0">Nemo enim ipsam voluptatem voluptas sit aspernatur ratione voluptatem sequi nesciunt..</p>
                                                                                </div>
                                                                            </div>
                                                                            <div className="card-body p-4 pl-5">
                                                                                <a className="mr-4" href="/"> <span className="font-weight-bold">Duration :</span> <span className="text-muted">6 Months</span></a>
                                                                                <a className="mr-4 float-right" href="/"><span className="font-weight-bold">Daily :</span><span className="text-muted"> 2 Hours </span></a>
                                                                            </div>
                                                                            <div className="card-body">
