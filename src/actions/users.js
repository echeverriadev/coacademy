...
export const REQUEST_REACTIVATE_USER_SUCCEDED =
  "REQUEST_REACTIVATE_USER_SUCCEDED";
export const REQUEST_REACTIVATE_USER_FAILED = "REQUEST_REACTIVATE_USER_FAILED";
export const REQUEST_UPLOAD_USER_IMAGE_SUCCEDED =
  "REQUEST_UPLOAD_USER_IMAGE_SUCCEDED";
export const REQUEST_UPLOAD_USER_IMAGE_FAILED =
  "REQUEST_UPLOAD_USER_IMAGE_FAILED";
      ...
      if (reg.test(userToSend.email) !== false) {
        if (userToSend.password != null && String(userToSend.password) !== "") {
          if (
            ...
            String(userToSend.confirm_password) !== ""
          ) {
            if (userToSend.password === userToSend.confirm_password) {
              if (userToSend.name != null && String(userToSend.name) !== "") {
                if (
                  userToSend.phone != null &&
                  String(userToSend.phone) !== ""
                ) {
                  if (
                    userToSend.sex !== null &&
                    String(userToSend.sex) !== "" &&
                    String(userToSend.sex) !== "-1"
                  ) {
                    if (
                      userToSend.role_id !== null &&
                      String(userToSend.role_id) !== "" &&
                      String(userToSend.role_id) !== "-1"
                    ) {
                      if (
                        user.isTeacher &&
                        (userToSend.description === null ||
                          String(userToSend.description) === "")
                      ) {
              ...
              message(
                "La contraseña y la de confirmación deben coincidir",
                "error",
                0
              );
            }
...
export function reactivateUser(id) {
  return function (dispatch, getState) {
    ...
    return fetch(
      process.env.REACT_APP_NODE_URL + "/users/" + id + "/reactivate",
      {
        method: "POST",
        mode: "cors",
        credentials: "with-credentials",
