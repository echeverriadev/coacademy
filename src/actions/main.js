...
export const HANDLE_RECOVER_PASSWORD_EMAIL_CHANGE =
  "HANDLE_RECOVER_PASSWORD_EMAIL_CHANGE";
...
export const REQUEST_UPLOAD_PROFILE_IMAGE_SUCCEDED =
  "REQUEST_UPLOAD_PROFILE_IMAGE_SUCCEDED";
export const REQUEST_UPLOAD_PROFILE_IMAGE_FAILED =
  "REQUEST_UPLOAD_PROFILE_IMAGE_FAILED";
export const REQUEST_UPDATE_PROFILE_SUCCEDED =
  "REQUEST_UPDATE_PROFILE_SUCCEDED";
export const REQUEST_UPDATE_PROFILE_FAILED = "REQUEST_UPDATE_PROFILE_FAILED";
export const REQUEST_CHANGE_PASSWORD_SUCCEDED =
  "REQUEST_CHANGE_PASSWORD_SUCCEDED";
export const REQUEST_CHANGE_PASSWORD_FAILED = "REQUEST_CHANGE_PASSWORD_FAILED";
export const REQUEST_RECOVER_PASSWORD_SUCCEDED =
  "REQUEST_RECOVER_PASSWORD_SUCCEDED";
export const REQUEST_RECOVER_PASSWORD_FAILED =
  "REQUEST_RECOVER_PASSWORD_FAILED";
...
export const REQUEST_REGION_COUNTRY_SUCCEDED =
  "REQUEST_REGION_COUNTRY_SUCCEDED";
...
function validateOrigin(user) {
  ...
  if (
    ...
    String(user.user_profile.country) === "" ||
    String(user.user_profile.country) === "-1"
  )
    result = false;
  if (
    !user.user_profile ||
    !user.user_profile.region ||
    String(user.user_profile.region) === "" ||
    String(user.user_profile.region) === "-1"
  )
    result = false;
  if (
    !user.user_profile ||
    !user.user_profile.city ||
    String(user.user_profile.city) === "" ||
    String(user.user_profile.city) === "-1"
  )
    result = false;
...
function validateUserWorks(user) {
  ...
  if (
    ...
    String(user.user_profile.charge) === ""
  )
    result = false;
  if (
    !user.user_profile ||
    !user.user_profile.area ||
    String(user.user_profile.area) === ""
  )
    result = false;
  if (
    !user.user_profile ||
    !user.user_profile.company ||
    String(user.user_profile.company) === ""
  )
    result = false;
      ...
      if (reg.test(user.email) !== false) {
        if (user.password != null && String(user.password) !== "") {
          if (
            ...
            String(user.confirm_password) !== ""
          ) {
            if (String(user.password) === String(user.confirm_password)) {
              if (
                user.user_profile &&
                user.user_profile.name !== null &&
                String(user.user_profile.name) !== ""
              ) {
                if (
                  user.user_profile &&
                  user.user_profile.phone !== null &&
                  String(user.user_profile.phone) !== ""
                ) {
                  if (
                    user.user_profile &&
                    user.user_profile.sex !== undefined &&
                    user.user_profile.sex !== null &&
                    String(user.user_profile.sex) !== "" &&
                    String(user.user_profile.sex) !== "-1"
                  ) {
                    if (validateOrigin(user)) {
                      if (validateUserWorks(user)) {
                        const country = csc.getCountryById(
                          user.user_profile.country
                        );
                        const region = csc.getStateById(
                          user.user_profile.region
                        );
                        ...
                        return fetch(
                          process.env.REACT_APP_NODE_URL + "/register",
                          {
                            method: "POST",
                            mode: "cors",
                            credentials: "with-credentials",
              ...
              message(
                "La contraseña debe ser igual a la de confirmación ",
                "error",
                200
              );
            }
...
export function getCoursesByUser() {
  return function (dispatch, getState) {
    ...
    return fetch(
      process.env.REACT_APP_NODE_URL + "/courses/byUser/" + user.id,
      {
        method: "GET",
        mode: "cors",
        credentials: "with-credentials",
...
export function updateProfile(user) {
  ...
  return function (dispatch, getState) {
    ...
    if (
      userToSend.phone !== "" &&
      userToSend.phone !== null &&
      userToSend.phone !== undefined
    ) {
      dispatch(requestMain());
...
export function changePassword(user) {
  ...
  return function (dispatch, getState) {
    if (
      ...
      if (
        userToSend.new_password !== "" &&
        userToSend.new_password !== null &&
        userToSend.new_password !== undefined
      ) {
        if (
          userToSend.confirm_password !== "" &&
          userToSend.confirm_password !== null &&
          userToSend.confirm_password !== undefined
        ) {
          if (userToSend.new_password === userToSend.confirm_password) {
            ...
            message(
              "La nueva contraseña y la de confirmación deben ser iguales",
              "error",
              0
            );
          }
...
export function _getRegionCountryApi(country_id) {
  ...
  return function (dispatch) {
    dispatch(
      requestRegionCountriesSucceded(csc.getStatesOfCountry(country_id))
    );
  };
}
