...
import { addDay, addDayWithoutFormat, dateParse} from "../components/utils/dateParse";
...
export const REQUEST_REACTIVATE_COURSE_SUCCEDED = 'REQUEST_REACTIVATE_COURSE_SUCCEDED'
export const REQUEST_REACTIVATE_COURSE_FAILED = 'REQUEST_REACTIVATE_COURSE_FAILED'
export const REQUEST_UPLOAD_COURSE_IMAGE_SUCCEDED = 'REQUEST_UPLOAD_COURSE_IMAGE_SUCCEDED'
export const REQUEST_UPLOAD_COURSE_IMAGE_FAILED = 'REQUEST_UPLOAD_COURSE_IMAGE_FAILED'
...
export const REQUEST_GET_COURSES_BY_CATEGORIES_SUCCEDED = 'REQUEST_GET_COURSES_BY_CATEGORIES_SUCCEDED'
export const REQUEST_GET_COURSES_BY_CATEGORIES_FAILED = 'REQUEST_GET_COURSES_BY_CATEGORIES_FAILED'
export const REQUEST_GET_COURSES_BY_MODALITIES_SUCCEDED = 'REQUEST_GET_COURSES_BY_MODALITIES_SUCCEDED'
export const REQUEST_GET_COURSES_BY_MODALITIES_FAILED = 'REQUEST_GET_COURSES_BY_MODALITIES_FAILED'
export const REQUEST_MARK_COURSE_AS_POPULAR_SUCCEDED = 'REQUEST_MARK_COURSE_AS_POPULAR_SUCCEDED'
...
function requestGetCoursesByModalitiesSucceded(courses){
	return {
		type: REQUEST_GET_COURSES_BY_MODALITIES_SUCCEDED,
		courses
	}
}

function requestGetCoursesByModalitiesFailed(){
	return {
		type: REQUEST_GET_COURSES_BY_MODALITIES_FAILED
	}
}
...
export function coursePriceWay(course){
	...
	if(course){
		if(parseInt(course.is_in_offer, 10) === 1)
			return "is_in_offer"
		else{
			if(parseInt(course.is_in_offer, 10) === 2 && parseFloat(course.price) > 0)
				return "isPaid"
			else
				return "isFree"
		}
...
export function courseDateWay(course){
	...
	if(course){
		if(parseInt(course.with_date, 10) === 1)
			return "with_date"
		else
			return "without_date"
	}
...
export function filterByCategory(category_id){
	...
	return function(dispatch, getState) {
		dispatch(requestCourses())
		const user = getState().user.userData
	    return fetch(process.env.REACT_APP_NODE_URL+"/courses/categories/" + category_id + "/filter",{
	        method: "GET",
	        mode: "cors",
	        credentials: "with-credentials",
			    headers: {
...
function validateDinamicFields(course){
	...
	if(course.priceWay === "is_in_offer"){
		...
		if(course.price === null || course.price === 0 || String(course.price) === "0"){
			message('Ingrese el precio base del curso.', 'error');
			return false;
		}

		if(course.offer_price === null || course.offer_price === 0 || String(course.offer_price) === "0"){
			message('Ingrese el precio de oferta del curso.', 'error');
			return false;
		}

		if(parseFloat(course.price) <= parseFloat(course.offer_price)){
			message('El precio de oferta no puede superar o ser igual al precio base.', 'error');
			return false;
		}
	...
	if(course.dateWay === "with_date"){
		if(course.begin_date === null || String(course.begin_date) === "" || course.end_date === null || String(course.end_date) === "" ){
			message('Ingrese las fechas indicadas');
			return false;							
		}

		if(addDayWithoutFormat(course.begin_date) < addDayWithoutFormat(dateParse(new Date())) || addDayWithoutFormat(course.begin_date) > addDayWithoutFormat(course.end_date)){
...
export function saveCourse(course){
	...
	return function(dispatch, getState){
		...
		let courseToSend = {
			...
			offer_price: (course.priceWay === "is_in_offer")? course.offer_price : null,
		...
		console.log(addDayWithoutFormat(course.begin_date), addDayWithoutFormat(course.end_date))

		if(courseToSend.name !== null && String(courseToSend.name) !== ""){
			if(courseToSend.category_id !== null && courseToSend.category_id !== "" &&  String(courseToSend.category_id) !== '-1'){
				if(courseToSend.modality_id !== null && courseToSend.modality_id !== "" &&  String(courseToSend.modality_id) !== '-1'){
					if(courseToSend.duration !== 0 && courseToSend.duration !== null && courseToSend.duration !== ""){
						if(courseToSend.provider !== null && String(courseToSend.provider) !== "" && String(courseToSend.provider) !== "-1"){
							if(courseToSend.price !== null && validateDinamicFields(course)){
								if(course.image !== undefined && course.image !== null &&  String(course.image) !== "" || (Object.entries(course.image).length !== 0 &&  course.image.constructor !== Object ) ){
									if(courseToSend.link_media !== null && String(courseToSend.link_media) !== ""){
										if(validateDinamicFields(course)){
												...
												if(course.document_description !== null){
													if(course.document_description.error){
														message('El archivo PDF excede el tamaño permitido (10 MB)', 'error', 0)
														return;
													}
													dataWithFile.append('document_description', course.document_description)
												}

												if(course.image !== null){
													if(course.image.error){
														message('La imagen excede el tamaño permitido (2 MB)', 'error', 0)
														return;
													}
												...
												.then(json => {
													...
													if (json && json.status !== 500) {
														...
														if(json !== undefined){
															...
															message('Hubo un error y no se pudo publicar el curso.', 'error')
														}
													}
												}
												)
...
export function getCoursesByCategories(){
	...
	return function(dispatch, getState) {
	    ...
	    return fetch(process.env.REACT_APP_NODE_URL+"/courses/categories/filter",{
	        method: "GET",
	        mode: "cors",
	        credentials: "with-credentials",
			    headers: {
...
export function getCoursesByModality(modality_id){
	...
	return function(dispatch, getState) {
	    ...
	    return fetch(process.env.REACT_APP_NODE_URL+`/courses/modality/${modality_id}/filter`,{
	        method: "GET",
	        mode: "cors",
	        credentials: "with-credentials",
			    headers: {
...
export function downloadPdf(file_name){
	...
	return function(dispatch){
	    ...
	    return fetch(process.env.REACT_APP_NODE_URL+"/uploads/courses/pdfs/"+ file_name + "/download",{
	        method: "GET",
	        mode: "cors",
	        credentials: "with-credentials",
			    headers: {
...
export function markAsPopular(course){
	...
	return function(dispatch, getState){
	   ...
	   dispatch(requestCourses())
	    return fetch(process.env.REACT_APP_NODE_URL+"/courses/"+ course.id + "/markAsPopular",{
	        method: "PUT",
	        mode: "cors",
	        credentials: "with-credentials",
	        headers:{
...
export function dismarkAsPopular(course){
	...
	return function(dispatch, getState){
	   ...
	   dispatch(requestCourses())
	    return fetch(process.env.REACT_APP_NODE_URL+"/courses/"+ course.id + "/dismarkAsPopular",{
	        method: "PUT",
	        mode: "cors",
	        credentials: "with-credentials",
	        headers:{
...
export function reactivateCourse(course_id){
	...
	return function(dispatch, getState){
	   ...
	   dispatch(requestCourses())
	    return fetch(process.env.REACT_APP_NODE_URL+"/courses/"+ course_id + "/reactivate",{
	        method: "PUT",
	        mode: "cors",
	        credentials: "with-credentials",
	        headers:{
